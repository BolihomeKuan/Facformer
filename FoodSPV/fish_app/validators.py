...
import numpy as np
import pandas as pd
from fish_app.models import FeatureData
...
def validate_import_data(df):
    ...
    # 检查必需列
    required_columns = ['Fish Species', 'Temperature', 'Storage Time', 
                       'Feature', 'Value', 'Day']
    ...
    # 检查数据一致性
    if 'Fish Species' in df.columns and 'Temperature' in df.columns:
        duplicates = df.groupby(['Fish Species', 'Temperature', 'Day', 'Feature'])\
