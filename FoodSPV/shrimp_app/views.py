...
from .forms import ShrimpForm, StorageConditionForm, FeatureDataForm, DataImportForm, ShrimpSourceForm
import numpy as np
...
from django.contrib.auth.decorators import login_required
from .forms import ShrimpForm, BulkFeatureDataForm,StorageConditionForm, FeatureDataForm, DataImportForm, ShrimpSourceForm


@require_http_methods(["GET"])
def index(request):
    """Shrimp database homepage with both card and table views"""
    shrimp_species = Shrimp.objects.all().order_by('name')
    ...
    return render(request, 'shrimp_app/index.html', context)
...
def dashboard(request):
    """显示仪表板"""
    # 保留原有的统计数据
    total_species = Shrimp.objects.count()
    ...
    # 添加鱼类统计数据
    shrimp_species = Shrimp.objects.all()
    shrimp_labels = [shrimp.name for shrimp in shrimp_species]
    shrimp_measurements = [FeatureData.objects.filter(
        storage_condition__shrimp_species=shrimp).count() 
        for shrimp in shrimp_species
    ]
    ...
    context = {
        ...
        'shrimp_labels': shrimp_labels,
        'shrimp_data': shrimp_measurements,
    }
    return render(request, 'shrimp_app/dashboard.html', context)
...
def statistical_analysis(request):
    ...
    return render(request, 'shrimp_app/statistical_analysis.html', context)
...
def advanced_search(request):
    ...
    return render(request, 'shrimp_app/advanced_search.html', context)
...
def compare_data(request):
        ...
        if analysis_type == 'single':
            ...
            if feature_data.exists():
                ...
                if len(values) > 0:
                    ...
                    # 准备单系列分析数据
                    single_analysis = {
                        'label': f"{feature_data[0].storage_condition.shrimp_species.name} ({feature_data[0].storage_condition.temperature}°C)",
                        'days': days.tolist(),
            ...
            for data in feature_data:
                key = f"{data.storage_condition.shrimp_species.name} ({data.storage_condition.temperature}°C)"
    ...
    return render(request, 'shrimp_app/compare_data.html', context)
...
def compare_analysis(request):
    ...
    if not feature_type_id or len(condition_ids) < 2:
        ...
        return redirect('shrimp_app:statistical_analysis')
    ...
    for condition_id in condition_ids:
        ...
        if data:
            ...
            comparison_datasets.append({
                'label': f'{storage.shrimp_species.name} ({storage.temperature}°C)',
                'data': values,
...
def shrimp_detail(request, shrimp_id):
    """Display detailed information for a specific shrimp species"""
    shrimp = get_object_or_404(Shrimp, id=shrimp_id)
    sources = shrimp.sources.all().prefetch_related('storage_conditions')
    
    context = {
        'shrimp': shrimp,
        'sources': sources
    }
    return render(request, 'shrimp_app/shrimp_detail.html', context)

@require_http_methods(["GET"])
def shrimp_list(request):
    """Display shrimp species in table format"""
    shrimp_species = Shrimp.objects.all().order_by('name')
    context = {
        'shrimp_species': shrimp_species,
        'storage_conditions_count': StorageCondition.objects.count(),
    ...
    return render(request, 'shrimp_app/shrimp_list.html', context)


@require_http_methods(["GET", "POST"])
@login_required
def add_shrimp(request):
    """添加新的鱼类"""
    if request.method == 'POST':
        form = ShrimpForm(request.POST, request.FILES)
        if form.is_valid():
            form.save()
            messages.success(request, 'Successfully added new shrimp species.')
            return redirect('shrimp_app:index')
    else:
        form = ShrimpForm()
    context = {'form': form}
    return render(request, 'shrimp_app/add_shrimp.html', context)

@require_http_methods(["GET", "POST"])
@login_required
def edit_shrimp(request, shrimp_id):
    """编辑鱼类信息"""
    shrimp = get_object_or_404(Shrimp, id=shrimp_id)
    if request.method == 'POST':
        form = ShrimpForm(request.POST, request.FILES, instance=shrimp)
        if form.is_valid():
            form.save()
            messages.success(request, 'Successfully updated shrimp species.')
            return redirect('shrimp_app:shrimp_detail', shrimp_id=shrimp.id)
    else:
        form = ShrimpForm(instance=shrimp)
    context = {'form': form, 'shrimp': shrimp}
    return render(request, 'shrimp_app/edit_shrimp.html', context)

@login_required
def delete_confirm(request, shrimp_id):
    shrimp = get_object_or_404(Shrimp, pk=shrimp_id)
    return render(request, 'shrimp_app/delete_confirm.html', {'shrimp': shrimp})

@login_required
@require_POST
def delete_shrimp(request, shrimp_id):
    shrimp = get_object_or_404(Shrimp, pk=shrimp_id)
    shrimp.delete()
    return redirect('shrimp_app:shrimp_list')
...
@require_http_methods(["GET", "POST"])
@login_required
def add_storage(request, shrimp_id, source_id):
    """添加储存条件"""
    shrimp = get_object_or_404(Shrimp, id=shrimp_id)
    source = get_object_or_404(ShrimpSource, id=source_id)
    
    if request.method == 'POST':
        form = StorageConditionForm(request.POST)
        if form.is_valid():
            storage = form.save(commit=False)
            storage.shrimp_species = shrimp
            storage.shrimp_source = source
            storage.save()
            messages.success(request, 'Successfully added storage condition.')
            return redirect('shrimp_app:shrimp_source_detail', shrimp_id=shrimp_id, source_id=source_id)
    else:
    ...
    return render(request, 'shrimp_app/add_storage.html', context)
...
def edit_storage(request, storage_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('shrimp_app:storage_detail', storage_id=storage.id)
    else:
        form = StorageConditionForm(instance=storage)
    context = {'form': form, 'storage': storage}
    return render(request, 'shrimp_app/edit_storage.html', context)
...
def storage_detail(request, storage_id):
    ...
    return render(request, 'shrimp_app/storage_detail.html', context)
...
def delete_storage(request, storage_id):
    """删除储存条件"""
    storage = get_object_or_404(StorageCondition, id=storage_id)
    shrimp_id = storage.shrimp_species.id
    storage.delete()
    messages.success(request, 'Successfully deleted storage condition.')
    return redirect('shrimp_app:shrimp_detail', shrimp_id=shrimp_id)
...
def add_feature_data(request, storage_id):
    ...
    context = {'form': form, 'storage': storage}
    return render(request, 'shrimp_app/add_feature_data.html', context)
@require_http_methods(["POST"])
@login_required
def delete_feature_data(request, feature_id):
    ...
    return redirect('shrimp_app:storage_detail', storage_id=storage_id)
...
def feature_detail(request, storage_id, feature_id):
    ...
    return render(request, 'shrimp_app/feature_detail.html', context)
...
def edit_feature_data(request, feature_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('shrimp_app:storage_detail', storage_id=feature_data.storage_condition.id)
    else:
    ...
    return render(request, 'shrimp_app/edit_feature_data.html', context)
@require_http_methods(["GET", "POST"])
@login_required
def bulk_add_feature_data(request, storage_id):
    ...
    return render(request, 'shrimp_app/bulk_add_feature_data.html', context)
...
def export_feature_data(request, storage_id):
    ...
    for data in feature_data_query:
        writer.writerow([
            ...
            storage.shrimp_species.scientific_name,
        ...
        if form.is_valid():
            try:
                ...
                for _, row in df.iterrows():
                    shrimp, _ = Shrimp.objects.get_or_create(
                        name=row['Shrimp Species'],
                        defaults={
                ...
                return redirect('shrimp_app:dashboard')
            except Exception as e:
                messages.error(request, f'Error importing data: {str(e)}')
    else:
        form = DataImportForm()
    
    return render(request, 'shrimp_app/import_data.html', {'form': form})
...
def bulk_edit(request):
    ...
    if request.method == 'POST':
        ...
        if not selected_items:
            ...
            return redirect('shrimp_app:bulk_edit')
    ...
    feature_data = FeatureData.objects.select_related(
        'storage_condition__shrimp_species',
        'feature_type'
    ).all()
    ...
    return render(request, 'shrimp_app/bulk_edit.html', context)
...
@login_required
def export_data(request, shrimp_id):
    """导出鱼类数据"""
    shrimp = get_object_or_404(Shrimp, id=shrimp_id)
    
    # 创建 HTTP 响应对象，设置文件类型和文件名
    response = HttpResponse(content_type='text/csv')
    filename = f"{shrimp.name}_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
@require_http_methods(["GET"])
def shrimp_source_detail(request, shrimp_id, source_id):
    """View for shrimp source details"""
    shrimp = get_object_or_404(Shrimp, id=shrimp_id)
    source = get_object_or_404(ShrimpSource, id=source_id)
    storage_conditions = StorageCondition.objects.filter(
        shrimp_species=shrimp,
        shrimp_source=source
    ).select_related('shrimp_species', 'shrimp_source')
    ...
    return render(request, 'shrimp_app/shrimp_source_detail.html', context)
...
@require_http_methods(["GET", "POST"])
@login_required
def add_source(request, shrimp_id):
    """添加鱼类来源"""
    shrimp = get_object_or_404(Shrimp, id=shrimp_id)
    
    if request.method == 'POST':
        form = ShrimpSourceForm(request.POST)
        if form.is_valid():
            source = form.save(commit=False)
            source.shrimp_species = shrimp
            source.save()
            messages.success(request, 'Successfully added shrimp source.')
            return redirect('shrimp_app:shrimp_detail', shrimp_id=shrimp.id)
    else:
        form = ShrimpSourceForm()
        # 移除这行代码，因为shrimp_species字段应该在表单的Meta类中被排除
        # form.fields['shrimp_species'].widget = forms.HiddenInput()
    
    context = {'form': form, 'shrimp': shrimp}
    return render(request, 'shrimp_app/add_source.html', context)
...
def edit_source(request, source_id):
    """Edit shrimp source"""
    source = get_object_or_404(ShrimpSource, id=source_id)
    shrimp_id = source.shrimp_species.id
    if request.method == 'POST':
        form = ShrimpSourceForm(request.POST, instance=source)
        if form.is_valid():
            form.save()
            messages.success(request, 'Successfully updated source.')
            return redirect('shrimp_app:shrimp_source_detail', shrimp_id=shrimp_id, source_id=source.id)
    else:
        form = ShrimpSourceForm(instance=source)
    context = {'form': form, 'source': source, 'shrimp': source.shrimp_species}  # 修改这里
    return render(request, 'shrimp_app/edit_source.html', context)


@require_http_methods(["POST"])
@login_required
def delete_source(request, source_id):
    """Delete shrimp source"""
    source = get_object_or_404(ShrimpSource, id=source_id)
    shrimp_id = source.shrimp_species.id
    source.delete()
