...
import numpy as np
import pandas as pd
from shrimp_app.models import FeatureData
...
def validate_import_data(df):
    ...
    # 检查必需列
    required_columns = ['Shrimp Species', 'Temperature', 'Storage Time', 
                       'Feature', 'Value', 'Day']
    ...
    # 检查数据一致性
    if 'Shrimp Species' in df.columns and 'Temperature' in df.columns:
        duplicates = df.groupby(['Shrimp Species', 'Temperature', 'Day', 'Feature'])\
